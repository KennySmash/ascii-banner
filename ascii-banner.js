class FlfAsciiRenderer extends HTMLElement {
  constructor() {
    super()
    this.attachShadow({ mode: 'open' })
    this.fontData = null
    this.fontKey = `banner-${Math.random().toString(36).slice(2, 9)}`
    ...
    this.scale = 1
    this.preElement.style.setProperty('--ascii-scale', 1)
  ...
  async connectedCallback() {
    ...
    if (this.shouldAutoFit() && 'ResizeObserver' in window && !this.resizeObserver) {
      this.resizeObserver = new ResizeObserver(() => this.debouncedFitBanner())
  ...
  static get observedAttributes() {
    return ['text', 'font-url', 'size', 'font-family', 'color', 'fit']
  }
  ...
  resolveSize(value) {
    ...
    const sizes = {
      badge: 'clamp(1rem, 0.35vw + 0.85rem, 1.6rem)',
      mini: 'clamp(1.2rem, 0.4vw + 1rem, 2rem)',
      compact: 'clamp(1.4rem, 0.45vw + 1.2rem, 2.6rem)',
      standard: 'clamp(2.2rem, 0.9vw + 1.8rem, 4.4rem)',
      statement: 'clamp(3.4rem, 1.6vw + 2.8rem, 6.4rem)',
      heroic: 'clamp(4.6rem, 2.2vw + 3.8rem, 8.6rem)'
    }
  ...
  applyStyles() {
    ...
    const colorValue = (this.color || 'inherit').trim() || 'inherit'
    ...
    this.styleElement.textContent = `
      ...
      pre {
        ...
        font-size: calc(${sizeValue} * var(--ascii-scale, 1));
  ...
  async loadFont() {
    try {
      ...
      console.error('Error loading FLF font:', error)
    }
  }

  async renderAscii() {
  if (!this.fontData || !this.preElement) return
  figlet.parseFont(this.fontKey, this.fontData)
  figlet.text(this.text, { font: this.fontKey }, (err, result) => {
      if (err) {
        console.error('Error rendering ASCII:', err)
        return
      }
      this.preElement.textContent = result
      this.scale = 1
      this.preElement.style.setProperty('--ascii-scale', 1)
      if (this.shouldAutoFit()) {
        this.fitBanner()
  ...
  fitBanner() {
    ...
    // Reset to scale 1 before measuring to get true natural dimensions
    this.preElement.style.setProperty('--ascii-scale', 1)
    ...
    // Get available width from host element (which is now display: block, width: 100%)
    const hostRect = this.getBoundingClientRect()
    const availableWidth = hostRect.width - 2 // 2px buffer

    // For height, check parent's computed height (only used with fit="both" or fit="height")
    const parentRect = this.parentElement?.getBoundingClientRect()
    const availableHeight = parentRect?.height || 0

    // Determine which axes to constrain based on fit mode
    // fit="auto" or fit="width" = width only (default, most common)
    // fit="both" = both width and height
    // fit="height" = height only
    ...
    this.preElement.style.setProperty('--ascii-scale', nextScale)
  }
}

customElements.define('ascii-banner', FlfAsciiRenderer)
